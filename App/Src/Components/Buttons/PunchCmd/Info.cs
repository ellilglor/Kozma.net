...
public class Info(IEmbedHandler embedHandler, IPunchHelper punchHelper, IPunchTracker punchTracker) : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task ExecuteAsync(string choice)
    {
        ...
        var desc = choice switch
        {
            ...
            ComponentIds.PunchInfoOdds => $"{Format.Italics("These are the chances to get Unique Variants")}\n\n" +
            $"{Format.Bold("When rolling at Punch:")}\n- Low: ~ 73.17%\n- Medium: ~ 19.51%\n- High: ~ 4.87%\n- Very High/Maximum: ~ 2.45%\n\n" +
            $"{Format.Bold("When crafting:")}\n- 1/10 for 1 UV\n- 1/100 for 2 UVs\n- 1/1000 for 3 UVs",
            ComponentIds.PunchInfoGamble => Format.Italics("These buttons let you roll for additional Unique Variants."),
            ComponentIds.PunchInfoLock => Format.Italics("These buttons let you lock/unlock a Unique Variant."),
            _ => "Invalid choice"
        };
