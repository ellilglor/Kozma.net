...
public class Bookchance : InteractionModuleBase<SocketInteractionContext>
{
    private readonly IEmbedFactory _embedFactory;

    public Bookchance(IEmbedFactory embedFactory)
    {
        ...
        [Summary(description: "Amount of Black Kats you encountered"), MinValue(1)] int kats)
    {
        ...
        {
            _embedFactory.CreateField("Black Kat spawn:", "1/90 or 1.11%"),
            _embedFactory.CreateField("Book drop:", "1/250 or 0.4%"),
            _embedFactory.CreateField("Overall chance per Kat:", "1/25000 or 0.004%"),
        };

        var embed = _embedFactory.GetEmbed($"After killing {kats} Black {(kats > 1 ? "kats" : "kat")} you have a {chance}% chance of getting at least 1 Book of Dark Rituals.")
